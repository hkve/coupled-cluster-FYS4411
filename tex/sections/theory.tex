...
Performing this minimization, we achieve an eigenvalue problem with the matrix
\begin{align}
    \hamhf{\alpha\beta} = \hshort{\alpha}{\beta} + \sum_{\gamma\delta} \rho_{\gamma\delta} \elmASshort{\alpha\gamma}{\beta \delta}, \hspace{20px} \rho_{\gamma\delta} = \sum_i C^*_{\gamma i} C_{\delta i} 
\end{align}
having the coefficients as eigenvectors and the Lagrange multipliers as eigenvalues
\begin{align}
    \sum_{\beta} \hamhf{\alpha \beta} C_{i \beta} = \epsilon_i C_{i \alpha}.
\end{align}
\comment{method?} This is solved iteratively, starting with $C_{ii} = 1$ \footnote{Note that this is over occupied states}, repeatedly diagonalizing $C$. As a stopping criteria, the sum of Lagrange multipliers per occupied states between iterations is used. 
...
\end{align*}
only gives non-zero contributions when both the spins of $p,r$ and $q,s$ align. This reduces 16 equal (or vanishing) elements to a single element, improving both storage and computation time.