...
Going back to the amplitude equation \cref{eq:theo:Amplitude_CCD_explicit} we separate the diagonal and off-diagonal part of $f_{pq}$ as $f_{p}^D$ and $f_{pq}^O$ respectively
...
\begin{align*}
    \permoperator{ab} f_{bc} \amplitude{ac}{ij} = f_{bc}\amplitude{ac}{ij} - f_{ac}\amplitude{bc}{ij},
\end{align*}
we can insert the diagonal and off-diagonal partition of the Fock matrix giving

\begin{align*}
    t_{bc} \amplitude{ac}{ij} &= \delta_{bc}f_{bb}\amplitude{ac}{ij} + (1-\delta_{bc})f_{bc}\amplitude{bc}{ij} \\
    &= f_{b}^D \amplitude{ab}{ij} + f_{bc}^O \amplitude{ac}{ij}
\end{align*}
Similarly 
\begin{align*}
    f_{ac}\amplitude{bc}{ij} = -f_a^D \amplitude{ab}{ij} + f_{ac}^O \amplitude{bc}{ij}
\end{align*}
...
\begin{align*}
    \permoperator{ab}f_{bc}\amplitude{ac}{ij} = (f_a^D + f_b^D)\amplitude{ab}{ij} + \permoperator{ab}f_{bc}\amplitude{ac}{ij}
\end{align*}
...
\begin{align}
    \amplitude{ab,(n+1)}{ij} = p\amplitude{ab,(n)}{ij} + (1-p)\frac{R^{ab}_{ij}(t^{(n)})}{\mathcal{F}^{ab}_{ij}} \label[eq]{eq:met:amplitudes_update_rule}
\end{align}
...
\begin{algorithm}[H]
    \begin{algorithmic}
        ...
        \EndWhile
    \end{algorithmic}
    \caption{Outline of Hartree-Fock iterative scheme.} \label[algo]{algo:met:ccd}
\end{algorithm}
