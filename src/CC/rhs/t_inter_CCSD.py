...
def amplitudes_CCSD_t1_t2(t1, t2, u, f, v, o):
    ...
    tau6 += np.einsum(
        "kjab,kiab->ij", tau0, u[o, o, v, v], optimize=True
    )

    tau14 = np.zeros((N, N, N, M))

    tau14 += np.einsum(
        "ijbc,kabc->ijka", tau0, u[o, v, v, v], optimize=True
    )

    tau16 = np.zeros((N, N, N, M))

    tau16 += np.einsum(
        "kjia->ijka", tau14, optimize=True
    )
    ...
    tau41 += np.einsum(
        "lkab,jiab->ijkl", tau0, u[o, o, v, v], optimize=True
    )

    r1 = np.zeros((M, N))

    r1 += np.einsum(
        "jibc,jabc->ai", tau0, u[o, v, v, v], optimize=True
    ) / 2
    ...
    tau37 += np.einsum(
        "abjl,ilkb->ijka", t2, tau1, optimize=True
    )
    ...
    r1 -= np.einsum(
        "bajk,ijkb->ai", t2, tau2, optimize=True
    ) / 2
    ...
    tau26 += np.einsum(
        "ai,ja->ij", t1, tau4, optimize=True
    )

    tau27 = np.zeros((N, N, M, M))

    tau27 += np.einsum(
        "ik,abkj->ijab", tau26, t2, optimize=True
    )
    ...
    r1 += np.einsum(
        "jb,baji->ai", tau4, t2, optimize=True
    )
    ...
    r1 -= np.einsum(
        "aj,ji->ai", t1, tau6, optimize=True
    ) / 2
    ...
    tau9 += np.einsum(
        "acik,jkbc->ijab", t2, tau8, optimize=True
    )
    ...
    tau13 += np.einsum(
        "ac,cbij->ijab", tau12, t2, optimize=True
    )
    ...
    tau17 += np.einsum(
        "bk,kija->ijab", t1, tau16, optimize=True
    )
    ...
    r2 -= np.einsum(
        "ijab->abij", tau18, optimize=True
    ) / 2

    r2 += np.einsum(
        "ijba->abij", tau18, optimize=True
    ) / 2
    ...
    tau23 -= np.einsum(
        "bk,ikja->ijab", t1, tau22, optimize=True
    )
    ...
    tau30 += np.einsum(
        "ki,abkj->ijab", tau29, t2, optimize=True
    )
    ...
    tau34 -= np.einsum(
        "acjk,ikbc->ijab", t2, tau33, optimize=True
    )
    ...
    tau39 += np.einsum(
        "bk,ikja->ijab", t1, tau38, optimize=True
    )
    ...
    r2 += np.einsum(
        "ijab->abij", tau40, optimize=True
    )

    r2 -= np.einsum(
        "ijba->abij", tau40, optimize=True
    )

    r2 -= np.einsum(
        "jiab->abij", tau40, optimize=True
    )

    r2 += np.einsum(
        "jiba->abij", tau40, optimize=True
    )
    ...
    r2 += np.einsum(
        "klba,klji->abij", tau0, tau41, optimize=True
    ) / 4
    ...
    r2 -= np.einsum(
        "jicd,bacd->abij", tau42, u[v, v, v, v], optimize=True
    ) / 2

    tau42 = None
    
    r1 -= np.einsum(
        "bj,jaib->ai", t1, u[o, v, o, v], optimize=True
    )
